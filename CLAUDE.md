...
- 0.8 requires a one-time `scan youtube-likes` to repopulate `artists` from `videoOwnerChannelTitle`. The **first** post-0.8 `compare-likes` will show a `metadata_drift` spike caused by the ingestion change itself; this spike won't clear on a re-run of `compare-likes` alone — verify it was migration noise by running a **second** `scan youtube-likes` + `compare-likes` so drift detection compares two post-0.8 snapshots.
