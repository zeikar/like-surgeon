...
- Re-running `sync` is idempotent: applied and skipped items are treated as terminal and not re-attempted; failures stay `'open'` and are retried (so lowering `--drift-min-confidence` will pick up borderline drifts on the next run).
- **Terminal statuses.** `'applied'` = every API call succeeded (or `ytm_dedupe` attempt regardless of outcome). `'skipped'` = code-set terminal non-failure outcome: either a planner-level skip (no video_id, below confidence threshold, unsupported duplicate shape) or a cross-prop verify-miss (`ytm_like` or `yt_like` where the expected platform didn't observe the like within 5s). `'skipped'` is also used for explicit manual overrides — e.g. a private/deleted YouTube ghost that `videos.rate` rejects with 403/404. Set `DiagnosisItem.status = 'skipped'` via SQL to permanently suppress a finding; flip it back to `'open'` to retry a code-set skip.
